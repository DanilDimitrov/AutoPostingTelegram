...
async def clone_content(client, source_channel_id: int, themes, source_channel_name: str, channel_go_to:str):
    try:
            ...
            if message.caption:
                try:
                    if not containsAD(message.caption.lower()) and validate_photo_and_len(message):
                        ...
                        description = generateText(query_to_GPT)
...
async def parse(channels_en_id: dict, channel_go_to):
    client = Client(name='client', api_id=API_ID, api_hash=API_HASH)
    await client.start()
    try:
        themes = get_themes()
        for channel_name, channel_id in channels_en_id.items():
            await clone_content(client, channel_id, themes, channel_name, channel_go_to)
    finally:
        await client.stop()
